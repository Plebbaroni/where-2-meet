...
async function generateIsochrones(coordinates : number[], transport : TransportationType, time : number) {
...
async function testStuff() {
    const data1 = await geocode('5 Gilmore Street, Cabramatta');
    const data2 = await generateIsochrones([51.507609,-0.128315], "public_transport", 30);
    console.log(data1);
    console.log(data2);
}
testStuff();
